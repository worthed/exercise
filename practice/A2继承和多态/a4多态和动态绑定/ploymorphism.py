...
def displayObject(g):
    # displayObject方法采用GeometricObject类型的参数。
    # 可以通过传递任何一个GeometricObject的实例（例如Circle(4)和Rectangle(1,3)
    # 来调用displayObject。使用父类对象的地方都可以使用子类的对象，即为多态
    print(g.__str__())
    # c是Circle类的一个对象。Circle类是GeometricObject类的子类。
    # __str__()方法在这两个类中都有定义。因此，displayObject方法中
